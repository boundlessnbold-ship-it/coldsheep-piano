...
YTDLP_COOKIES_FILE = os.environ.get("YTDLP_COOKIES_FILE")  # cookies.txt로 디코드된 경로(있으면 사용)
...
YTDLP_CLIENT_ARGS = ["--extractor-args", "youtube:player_client=tv,-web"]


def _auth_args():
    # 쿠키가 있으면 쿠키를 우선 사용 (가장 안정적), 없으면 클라이언트 스푸핑으로 폴백
    if YTDLP_COOKIES_FILE and os.path.exists(YTDLP_COOKIES_FILE):
        return ["--cookies", YTDLP_COOKIES_FILE]
    return list(YTDLP_CLIENT_ARGS)


def download_audio_original(video_url: str, out_stub: str) -> str:
    cmd = ["yt-dlp", "-f", "bestaudio"] + _auth_args()
    if YTDLP_PROXY:
...
def fetch_video_title(video_url: str) -> str:
    cmd = ["yt-dlp", "--print", "%(title)s", "--skip-download"] + _auth_args()
    if YTDLP_PROXY:
        cmd += ["--proxy", YTDLP_PROXY]
    cmd += [video_url]
    result = subprocess.run(cmd, capture_output=True, text=True)
